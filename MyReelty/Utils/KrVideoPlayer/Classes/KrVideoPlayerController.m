...
@property (nonatomic, assign) BOOL isLeftFullscreenMode;
@property (nonatomic, assign) BOOL isRightFullscreenMode;
...
- (void)fullScreenButtonClick
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(videoPlayerChangedVideoOriintation:)]) {
        [self.delegate videoPlayerChangedVideoOriintation: UIDeviceOrientationLandscapeRight];
    }
...
- (void)shrinkScreenButtonClick
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(videoPlayerChangedVideoOriintation:)]) {
        [self.delegate videoPlayerChangedVideoOriintation: UIDeviceOrientationPortrait];
    }
...
- (void)onDeviceOrientationChange{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(videoPlayerChangedVideoOriintation:)]) {
        [self.delegate videoPlayerChangedVideoOriintation: orientation];
    }
...
- (void)backOrientationPortrait{
    ...
    [UIView animateWithDuration:0.3f animations:^{
        ...
        NSLog(@"Portrait = %@",NSStringFromCGRect( self.frame));
...
- (void)setDeviceOrientationLandscapeRight{
    ...
    //        }];
    //    }
    if (self.isFullscreenMode && self.isLeftFullscreenMode) {
        return;
    }
        if (self.isRightFullscreenMode) {
            [self backOrientationPortrait];
        }
    ...
    [UIView animateWithDuration:0.3f animations:^{
        self.frame = frame;
        NSLog(@"Right = %@",NSStringFromCGRect( self.frame));
        [self.view setTransform:CGAffineTransformMakeRotation(M_PI_2)];
        
        self.isFullscreenMode = YES;
        self.isRightFullscreenMode = NO;
        self.isLeftFullscreenMode = YES;
    } completion:^(BOOL finished) {
...
- (void)setDeviceOrientationLandscapeLeft{
    ...
    //        }];
    //    }
    if (self.isFullscreenMode && self.isRightFullscreenMode) {
        return;
    }
    if (self.isLeftFullscreenMode) {
        [self backOrientationPortrait];
    }
    self.originFrame = self.view.frame;
    CGFloat height = [[UIScreen mainScreen] bounds].size.width;
    ...
    [UIView animateWithDuration:0.3f animations:^{
        self.frame = frame;
        NSLog(@"Left = %@",NSStringFromCGRect( self.frame)   );
        [self.view setTransform:CGAffineTransformMakeRotation(-M_PI_2)];
        
        self.isFullscreenMode = YES;
        self.isRightFullscreenMode = YES;
        self.isLeftFullscreenMode = NO;
    } completion:^(BOOL finished) {
...
- (UIView *)movieBackgroundView
{
    if (!_movieBackgroundView) {
        ...
        _movieBackgroundView.backgroundColor = [UIColor redColor];
    }
